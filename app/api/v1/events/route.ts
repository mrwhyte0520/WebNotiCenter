...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[v1] Error creating event notification:", error)
      return NextResponse.json(
        { error: "Failed to create event notification", details: error.message },
        { status: 500, headers: corsHeaders },
      )
    }

    return NextResponse.json({ success: true, notification: created }, { status: 201, headers: corsHeaders })
  } catch (error) {
